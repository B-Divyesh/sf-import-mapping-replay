...
Word counts use space-separated words. Interface labels and fragments are included where they carry meaning.

| Copy | Words | Result |
| --- | ---: | --- |
| Replay CSV mappings with proof | 5 | Pass |
...
| The CLI runs without internet. | 6 | Pass |
| Replay costs £0. | 3 | Pass |
...
| Rust 1.85 or newer builds the CLI. | 7 | Pass |
| No account is required. | 4 | Pass |
| It does not connect to a SaaS account. | 8 | Pass |
| It does not schedule or upload imports. | 7 | Pass |
...
| Refunds are handled by the merchant. | 6 | Pass |
| A refund revokes the license. | 5 | Pass |
| The free CLI does not need a license. | 8 | Pass |
...
Catalog description: “Replay customer CSV mappings with field-level evidence and validation.” (69 characters.)
